...
class TTSService:
    ...
    def synthesize_to_file(self,text,out_path,speaker_wav=None,language=None):
        ...
        if speaker_wav:
            self.tts.tts_with_vc_to_file(text=text,speaker_wav=speaker_wav,file_path=out_path,**extra)
        else:
            self.tts.tts_to_file(text=text,file_path=out_path,**extra)
